...
| `--mcp-port` | 50052 | MCP server port |
...
```bash
./artf-server --enable-grpc --grpc-port=50051
```

**All Services (Development)**
```bash
./artf-server --enable-grpc --enable-mcp --enable-web
```

**MCP Only (AI Integration)**
```bash
./artf-server --enable-mcp --mcp-port=50052
```
