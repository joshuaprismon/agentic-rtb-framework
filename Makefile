.PHONY: all build test clean generate docker run lint help
...
help:
	@echo "ARTF Agent Server - Makefile commands:"
	@echo ""
	@echo "Build Commands:"
...
## run: Run the server with default settings (gRPC only)
run: build
	@echo "Starting server (gRPC only)..."
	./$(BINARY_NAME) --enable-grpc --grpc-port=$(GRPC_PORT) --health-port=$(HEALTH_PORT)

## run-grpc: Run with gRPC server enabled
run-grpc: build
	@echo "Starting server with gRPC..."
	./$(BINARY_NAME) --enable-grpc --grpc-port=$(GRPC_PORT) --health-port=$(HEALTH_PORT)

## run-mcp: Run with MCP server enabled
run-mcp: build
	@echo "Starting server with MCP..."
...
run-web: build
	@echo "Starting server with Web UI and MCP..."
...
run-all: build
	@echo "Starting server with all services..."
